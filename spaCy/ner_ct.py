...
SKIP_TERMS = {
    ...
    # Standalone anatomical fragments that need modifiers
    'lobe', 'lobes', 'segment', 'segments', 'structures', 'vessels', 'tissue',
    'tissues', 'lung', 'lungs', 'bronchi', 'bronchus', 'sections', 'borders',
    # Procedural/measurement terms
    'widths', 'calibration', 'caliber', 'diameters', 'heights', 'diameter',
    'optimally', 'evaluated', 'suboptimal', 'absence', 'presence',
    # Directional terms that shouldn't appear alone
    'upper', 'lower', 'left', 'right', 'mid', 'medial', 'lateral',
    # Generic state terms
    'open', 'patent', 'terminating', 'junction', 'context', 'history'
}
...
def is_valid_entity(text, label):
    ...
    # Skip standalone location/directional terms
    STANDALONE_LOCATION = {'left', 'right', 'upper', 'lower', 'mid', 'anterior', 
                           'posterior', 'medial', 'lateral', 'bilateral'}
    if text_lower in STANDALONE_LOCATION:
        return False
    
    # Skip standalone anatomical structures without modifiers (too generic)
    GENERIC_ANATOMY = {'trachea', 'bronchi', 'bronchus', 'lung', 'lungs', 'heart',
                       'aorta', 'esophagus', 'mediastinum', 'vertebra', 'bone',
                       'bones', 'structures', 'vessels', 'tissue'}
    if text_lower in GENERIC_ANATOMY:
        return False
...
def should_apply_negation(entity_text, label):
    """Check if negation makes semantic sense for this entity."""
    text_lower = entity_text.lower().strip()
    
    # Anatomical structures - negation doesn't make sense
    ANATOMY_TERMS = {
        'trachea', 'bronchi', 'bronchus', 'lung', 'lungs', 'heart',
        'esophagus', 'mediastinum', 'aorta', 'vertebra', 'bone', 'bones',
        'structures', 'vessels', 'sections', 'borders', 'optimally',
        'evaluated', 'absence', 'contrast material', 'material'
    }
    
    # Findings/pathologies - negation makes sense
    FINDING_TERMS = {
        'effusion', 'mass', 'nodule', 'infiltration', 'consolidation',
        'pneumothorax', 'lesion', 'thickening', 'enlargement', 'fluid',
        'collection', 'pathology', 'abnormality', 'fracture', 'opacity',
        'atelectasis', 'edema', 'congestion', 'cardiomegaly'
    }
    
    # Check if any anatomy term is in the entity (skip negation)
    for anat in ANATOMY_TERMS:
        if anat in text_lower:
            return False
    
    # Check if any finding term is in the entity (apply negation)
    for finding in FINDING_TERMS:
        if finding in text_lower:
            return True
    
    # For diseases/chemicals, negation makes sense
    if label in {'DISEASE', 'CHEMICAL'}:
        return True
    
    # Default: apply negation for multi-word phrases, skip for single words
    words = text_lower.split()
    return len(words) >= 2
...
def generate_concept(entity):
    ...
    # Add negation prefix ONLY if it makes semantic sense
    if negated and should_apply_negation(text, label):
        parts.append("no")
    elif negated:
        # Skip anatomical structures that are negated (e.g., "no trachea")
        return None
